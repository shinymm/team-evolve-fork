...
export class QVQModelService {
  ...
  async analyzeImage(
    ...
    try {
      console.log('QVQModelService开始分析图像，接收图片URL数量:', imageUrls.length);
      ...
      // 构建请求体
      const requestBody = {
        model: modelConfig.model || 'qvq-max',
        messages: messages,
        stream: true
      };
      ...
      while (retryCount <= maxRetries) {
        try {
          ...
          if (!response.ok) {
            ...
            throw new Error(`通义千问QVQ API请求失败: ${response.status} ${errorText}`);
          }
          
          // 转换QVQ模型响应，处理推理过程和答案内容
      ...
      console.error('调用通义千问QVQ API失败:', error);

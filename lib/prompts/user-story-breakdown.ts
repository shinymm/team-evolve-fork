...
请按照以下严格的YAML格式输出用户故事列表，注意保持格式紧凑，不要在各个部分之间添加多余的空行：
- feature: [功能名称1]
  stories:
    - story: [用户故事1]
...
7. 输出格式必须紧凑，不要在各个部分之间添加多余的空行，这会导致前端渲染问题
8. 确保每个字段都有值，不要输出空的字段
9. 特别注意：仔细分析需求中提到的所有业务规则和约束条件，将它们完整地转化为具体的验收标准。例如，如果需求中提到"满意度等级设置为五级且用户可修改但不得超过五级"，应在相关用户故事的验收标准中明确包含这些规则

请直接输出YAML格式的结果，不要包含任何其他解释性文字，不要使用markdown代码块，直接输出原始YAML。`; 
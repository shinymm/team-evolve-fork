...
请按照以下规则设计测试用例：
1. 使用表格形式输出，包含以下列：
- 用例类型（HappyPath、SadPath、ExceptionPath）
- 测试用例概述（15字内，简洁易懂）
- 前提条件（明确列出所有前提条件）
- 用例步骤(通常包含多个步骤，用1.2.3.4.5...表示；每个步骤只包含一个具体操作)
- 预期结果（预期结果要具体且可验证）
2. 测试覆盖要求：x
- 核心功能正常场景的用例
- 功能对应的边界条件的用例
...
<测试用例表格示例>
| 用例类型｜测试用例概述 | 前提条件 | 用例步骤 | 预期结果 | 
|---｜---------|-----------------|---------------|--------------|
| HappyPath｜外呼任务管理_成功更新线路数 | 用户已成功登录并有外呼任务管理权限 | 1. 点击“外呼任务管理”；2. 找到已完成任务并点击；3.查看任务详情并输入有效的线路数；4.点击“保存” | 线路数成功更新 | 
` 
...
请遵循以下规则：
1. 分析测试描述中的内容，识别并拆分其中的多个用例场景，然后对每个测试用例分别提取以下信息：
...
2. 格式化要求：
   ...
   - 预期结果要具体且可验证
   - 前提条件要完整且明确
3. 如果原始描述缺少某些信息，需要合理补充完整

请直接生成格式化后的测试用例表格，表格前后不要生成其他任何描述：

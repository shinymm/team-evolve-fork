...
export const generateFromStepsPromptTemplate = `作为一位专业的测试专家，请基于以下简要的操作路径，生成完整的测试用例细节。

操作路径：
{steps}

请完善以下内容：
1. 前提条件：分析并列出执行该测试路径所需的所有前置条件（充分考虑用户权限和角色、系统状态、外部依赖、软件版本、时间因素、资源限制等）
2. 详细的测试步骤：将简要路径扩展为具体的操作步骤，每个步骤应该是一个具体的操作，最好能浓缩在7步内
3. 预期结果：描述完成所有步骤后应该看到的结果

请使用JSON格式返回，包含以下字段：
- preconditions: 前提条件
- detailed_steps: 详细步骤（使用数字编号，每个步骤一行）
- expected_result: 预期结果

示例输出：
{
  "preconditions": "1. 系统正常运行\\n2. 用户已登录\\n3. 用户有相应的操作权限",
  "detailed_steps": "1. 点击左侧菜单"知识库管理"\\n2. 在知识库列表中找到目标知识库\\n3. 点击"添加意图"按钮\\n4. 在弹出的表单中填写意图名称\\n5. 填写意图描述\\n6. 点击"保存"按钮",
  "expected_result": "1. 新添加的意图出现在意图列表中\\n2. 系统提示"添加成功""
}`

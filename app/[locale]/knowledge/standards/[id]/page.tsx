...
export default function StandardPage({ params }: StandardPageProps) {
  ...
  const fetchStandard = async () => {
    ...
    try {
      ...
      if (!response.ok) throw new Error('获取规范详情失败')
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!response.ok) throw new Error('更新规范失败')
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (!response.ok) throw new Error('删除规范失败')
    ...
    <div className="min-h-[calc(100vh-3.5rem)] bg-gradient-to-b from-gray-50 to-gray-100 flex flex-col items-center">
      <div className="w-full max-w-7xl px-4 py-12">
        <div className="flex items-center mb-8">
          ...
          <h1 className="text-3xl font-bold text-gray-800 tracking-tight">{editMode ? t('editStandard') : standard?.name}</h1>
        ...
        {editMode ? (
          ...
          <Card className="bg-white shadow-sm">
            ...
            <CardFooter className="text-sm text-gray-500">
              <div>{t('createdAt')}: {new Date(standard?.createdAt || '').toLocaleString()}</div>
              <div className="ml-auto">{t('updatedAt')}: {new Date(standard?.updatedAt || '').toLocaleString()}</div>
            </CardFooter>
          </Card>
        )}
      </div>

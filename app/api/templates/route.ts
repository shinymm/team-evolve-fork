...
export async function GET(req: NextRequest) {
  try {
    ...
    console.log(`处理GET /api/templates请求, 系统ID: ${systemId}, 其他过滤条件:`, {name, tags: tags?.join(',')});
    
    const templates = await TemplateService.getTemplates(filters);
    console.log(`成功获取模板数据: ${templates.length}条记录`);
...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log('创建模板请求数据:', { name, systemId });
    ...
    console.error('创建模板失败，详细错误:', error);

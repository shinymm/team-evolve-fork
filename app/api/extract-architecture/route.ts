...
export async function POST(request: NextRequest) {
  try {
    console.log(`[${new Date().toISOString()}] API: 收到信息架构抽取请求`);
...
async function handleOpenAIFileRequest(
  ...
  // 构建消息，引用已上传的文件
  const messages = [
    { role: 'system', content: 'You are a helpful assistant that extracts information architecture from documents.' },
    // 引用所有文件
    ...fileIds.map(fileId => ({ role: 'system', content: `fileid://${fileId}` })),
    // 用户提示
    { role: 'user', content: requirementArchitecturePrompt.replace('{{document_content}}', '') }
  ];
...
async function handleGeminiFileRequest(
  ...
  try {
    ...
    // 添加提示文本
    contents.push(requirementArchitecturePrompt.replace('{{document_content}}', ''));

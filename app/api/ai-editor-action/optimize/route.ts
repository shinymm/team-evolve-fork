        ...
        if (line.startsWith('data: ')) {
          try {
            ...
            if (json.choices && json.choices[0]?.delta?.content) {
              ...
              controller.enqueue(encoder.encode(JSON.stringify({ optimizedText: content })));
            }
          } catch (e) {
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!config) {
      console.error('Optimize API Error: Default AI config not found.');
    ...
    if (!config.model) {
      console.error('Optimize API Error: Default AI config is missing the required \'model\' field.', config);
      return new Response(JSON.stringify(
    ...
    try {
        ...
        console.error('Optimize API Error: Failed to decrypt API key.', decryptionError);
        return new Response(JSON.stringify(
    ...
    // 准备产品知识整合
    const combinedProductKnowledge = `
产品概述：${productKnowledge.productOverview || "无产品概述信息。"}
用户画像：${productKnowledge.userPersonas || "无用户画像信息。"}
架构信息：${productKnowledge.architectureInfo || "无架构信息。"}
    `.trim();
    
    // 准备提示词数据
    const promptData = {
      reqBackground: fullText.substring(0, 1000), // 使用全文的前部分作为背景
      reqBrief: fullText.substring(0, 300), // 使用文档开头作为概述
      sceneContent: text, // 选中的文本作为场景内容
      productKnowledge: combinedProductKnowledge
    };
    ...
    console.log(`Optimize request using model: ${finalConfig.model} (Is Gemini: ${isGemini})`);
          ...
          if (isGemini) {
            try {
              ...
              for await (const chunk of textStream) {
                if (chunk.text) {
                  controller.enqueue(
                    encoder.encode(JSON.stringify({ optimizedText: chunk.text }))
                  );
                }
              }
              
            } catch (geminiError) {
              console.error('Gemini API Error during optimization:', geminiError);
              controller.enqueue(
            ...
            try {
              ...
              console.log(`Sending optimization stream request to standard endpoint: ${endpoint}`);
              ...
              const response = await fetch(endpoint, {
                ...
                body: JSON.stringify({
                  model: finalConfig.model,
                  messages: [
                    { role: 'system', content: '你是一位产品经理的AI助手，擅长优化需求场景描述' },
              ...
              if (!response.ok) {
                ...
                console.error(`Standard API Error during optimization (${response.status}): ${errorText}`);
                controller.enqueue(
                  encoder.encode(JSON.stringify({ 
              ...
              console.error('Standard API Error during optimization:', standardError);
              controller.enqueue(
                encoder.encode(JSON.stringify({ 
    ...
    console.error('Error in /api/ai-editor-action/optimize:', error);
    let errorMessage = 'Internal Server Error';
    let errorDetails = 'An unexpected error occurred.';
    if (error instanceof Error) {
      errorMessage = 'Failed to optimize scene requirements';

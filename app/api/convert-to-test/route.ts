...
export async function POST(request: NextRequest) {
  try {
    ...
    const { fileIds, files, apiConfig, requirementChapter } = await request.json()
    
    console.log(`[${new Date().toISOString()}] API: 原始请求参数:`, JSON.stringify({
      fileIds: fileIds,
      files: files,
      requirementChapter: requirementChapter,
      apiConfig: {
        ...apiConfig,
        apiKey: '***' // 隐藏API密钥
      }
    ...
    console.log(`[${new Date().toISOString()}] API: 模型类型: ${isGemini ? 'Gemini' : 'OpenAI兼容'}, 模型名称: ${model}`);
    ...
    const streamResponse = async () => {
      try {
        // 准备消息内容
        console.log(`[${new Date().toISOString()}] API: 处理请求，模型=${model}, 文件数=${normalizedFileIds.length}, 需求章节=${requirementChapter || '无'}`);
        
        if (isGemini) {
          // 处理Gemini模型的文件请求
          await handleGeminiFileRequest(normalizedFileIds, model, apiKey, writer, requirementChapter);
        } else {
          // 处理OpenAI兼容API的文件请求
          await handleOpenAIFileRequest(normalizedFileIds, model, apiKey, baseURL, writer, requirementChapter);
        }
...
async function handleGeminiFileRequest(
  ...
  try {
    ...
    // 添加提示文本
    contents.push(requirementToTestPrompt(requirementChapter));
